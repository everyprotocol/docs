...
// ai! put them in a table
In traditional token standards:
- **ERC20** tokens store balances per address.
- **ERC721** tokens map each token ID to an owner and optional metadata URI.
- **ERC1155** tokens combine both, supporting multiple types in one contract.

// ai! put object records in the same table, so that we can compare
In Every Protocol, each object has a **record** — a structured onchain representation of its content and behavior.
An object’s record can include:

- **Meta** – name, description, timestamps, and other common fields
- **Elements** – the components that make up the object (see below)
- **Connections** – links to other objects, sets, or external resources
- **Facets** – modular extensions that define how the object is rendered, interacted with, or computed
